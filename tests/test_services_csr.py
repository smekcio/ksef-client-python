...
class CsrServiceTests(unittest.TestCase):
    def test_build_subject(self):
        ...
        self.assertEqual(subject.get_attributes_for_oid(NameOID.ORGANIZATION_IDENTIFIER)[0].value, "ID")
        self.assertEqual(subject.get_attributes_for_oid(NameOID.SERIAL_NUMBER)[0].value, "SN")
        self.assertEqual(subject.get_attributes_for_oid(NameOID.X500_UNIQUE_IDENTIFIER)[0].value, "UID")
