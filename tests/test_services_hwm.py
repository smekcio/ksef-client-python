...
from ksef_client.services.hwm import dedupe_by_ksef_number, get_effective_start_date, update_continuation_point


class HwmServiceTests(unittest.TestCase):
    def test_update_continuation_point(self):
        points = {}
        update_continuation_point(points, "seller", {"isTruncated": True, "lastPermanentStorageDate": "2024-01-01"})
        self.assertEqual(points["seller"], "2024-01-01")

        update_continuation_point(points, "seller", {"isTruncated": False, "permanentStorageHwmDate": "2024-02-01"})
